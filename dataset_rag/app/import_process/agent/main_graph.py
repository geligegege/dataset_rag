...
def route_after_entry(state: ImportGraphState) -> str:
    """
    node_entry 之后的路由函数，根据输入文件类型决定流程走向。
    1. 如果 state 中标记了 is_pdf_read_enabled，则路由到 node_pdf_to_md。
    2. 如果 state 中标记了 is_md_read_enabled，则路由到 node_md_img。
    3. 否则，直接路由到 END，结束流程。
    """
...
#5、定义静态边
workflow.add_edge("node_pdf_to_md", "node_md_img")  # PDF转MD后需要处理图片
workflow.add_edge("node_md_img", "node_document_split")  # 处理完图片后进行文档分块
workflow.add_edge("node_document_split", "node_item_name_recognition")  # 分块后进行项目名识别
workflow.add_edge("node_item_name_recognition", "node_bge_embedding")  # 识别项目名后进行向量化
workflow.add_edge("node_bge_embedding", "node_import_milvus")  # 向量化后导入Milvus
workflow.add_edge("node_import_milvus", END)  # 导入Milvus后结束流程

#6、编译状态图
kb_import_app = workflow.compile()


def save_import_graph(output_dir: str | Path | None = None) -> dict[str, str]:
    """
    保存当前导入流程状态图。
    ...
    mermaid_path = graph_dir / "kb_import_state_graph.mmd"
    mermaid_path.write_text(graph.draw_mermaid(), encoding="utf-8")
    saved_paths["mermaid"] = str(mermaid_path)

    try:
        ascii_path = graph_dir / "kb_import_state_graph.txt"
    ...
    try:
        png_path = graph_dir / "kb_import_state_graph.png"
